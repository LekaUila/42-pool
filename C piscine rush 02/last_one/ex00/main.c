...
/*   Updated: 2022/07/31 19:05:10 by lflandri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_parsing(int argc, char *argv[])
{
	...
	if ((argc != 2 && argc != 3)
		|| (ft_strcmp(nbr_without_start_0(argv[argc - 1]), "4294967295") > 0
			&& ft_strlen(argv[argc - 1]) == 10)
		|| ft_strlen(nbr_without_start_0(argv[argc - 1])) > 10)
	{
		write(1, "Error\n", 6);
		return (1);
	}
...
int	starting_recursive(char *dict, char *nbr)
{
	...
	if (full_z(nbr) == 1)
	{
		ft_putnbr(ft_strstr(dict, "0", 0), &del_space);
		write(1, "\n", 1);
		return (0);
	}
