...
/*   Updated: 2022/07/31 14:50:46 by lflandri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_01_to_99(char *nbr, char *dic, char *del_space)
{
	...
	{
		ft_putnbr(ft_strstr(dic, ft_add0(1, nbr[1])), del_space);
		if (nbr[0] != '0')
		{
			ft_putnbr(ft_strchar(dic, nbr[0]), del_space);
		}
	...
	else
	{
		if (nbr[1] != '0')
		{
			temp[0] = nbr[1];
			temp[1] = nbr[0];
			ft_putnbr(ft_strstr(dic, temp), del_space);
		}
	}
}
...
void	ft_recursive_if(char *dic, char *nbr_r, int i, char *del_space)
{
	if (i % 3 == 2)
	{
		if (nbr_r[i] != '0')
		{
			...
			ft_putnbr(ft_strstr(dic, ft_add0(2, '1')), del_space);
		}
	}
	if (i % 3 == 1)
	{
...
void	ft_recursive_else(char *dic, char *nbr_r, int i, char *del_space)
{
		...
		|| nbr_r[i + 2] != '0')
	{
		...
		if (i != 0)
			ft_putnbr(ft_strstr(dic, ft_add0(i, '1')), del_space);
	}
}
